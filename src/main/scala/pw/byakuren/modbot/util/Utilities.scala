...
import pw.byakuren.modbot.commands.CommandPermission
...
object Utilities {
  ...
  implicit class MemberUtils(member: Member) {
    ...
    def permissionLevel(implicit guildDataManager: GuildDataManager): CommandPermission = {
      if (Main.ownerOption.isDefined && member.getIdLong == Main.ownerOption.get.getIdLong) return CommandPermission.Debug
      if (isGuildModerator) return CommandPermission.Admins
